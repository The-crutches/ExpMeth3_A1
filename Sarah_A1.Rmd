...
```{r}
LU <- read.csv('C:/Users/Sarah/OneDrive/Skrivebord/R_work/ExpMeth3_A1/LU_train.csv', header = T)
demo <- read.csv('C:/Users/Sarah/OneDrive/Skrivebord/R_work/ExpMeth3_A1/demo_train.csv', header = T)
token <- read.csv('C:/Users/Sarah/OneDrive/Skrivebord/R_work/ExpMeth3_A1/token_train.csv', header = T)
```
...
# creating a function that can be applied unto each df easier
...
#appliyng the function to the demo df
demo <- remove_dots(demo)
token <- remove_dots(token)
LU <- remove_dots(LU)
...
```{r}
demo <- subset(demo, select = c("SUBJ", "VISIT", "Ethnicity", "Diagnosis", "Gender", "Age", "ADOS", "MullenRaw", "ExpressiveLangRaw", "Socialization"))

LU <- subset(LU, select = c("SUBJ", "VISIT", "MOT_MLU", "CHI_MLU"))

token <- subset(token, select = c("SUBJ", "VISIT", "types_MOT", "types_CHI", "tokens_MOT", "tokens_CHI"))
```
...
```{r}
visit1 <- subset(df2, select=c("SUBJ", "VISIT","ADOS", "MullenRaw","ExpressiveLangRaw", "Socialization")) %>% filter(VISIT == '1')


#renaming variables so i can merge the visit1 with df2 
...
df <- merge(visit1_r, df2)

#renaming variables for easy recall
df <- rename(df, c('nonverbalIQ' = 'MullenRaw', 'nonverbalIQ1' = 'MullenRaw1', 'verbalIQ' = 'ExpressiveLangRaw', 'verbalIQ1' = 'ExpressiveLangRaw'))
...
#is it?
df$Gender <- str_replace_all(df$Gender, c('1' = 'M', '2' = 'F'))

df$Diagnosis <- str_replace_all(df$Diagnosis, c('A$' = 'ASD', 'B' = 'TD'))


write.csv(df, file = 'autism_df')
```
...
```{r}
high_mlu <- filter(df, df$CHI_MLU >= 2.7)
low_mlu_1 <- filter(df, df$CHI_MLU <= 1.5 & df$VISIT == '1')

trials <- filter(df, df$VISIT != 6)
...
```{r}
most_words <- arrange(df, desc(tokens_CHI)) %>% filter(VISIT == '6')
least_words <- arrange(df, tokens_CHI) %>% filter(VISIT == '1')
...
```{r}
asd <- select(df, 'SUBJ', 'Diagnosis', 'CHI_MLU', 'tokens_CHI') %>% filter(Diagnosis == 'ASD')

test <- select(df, 'SUBJ', 'VISIT', 'VISIT')
view(test)
#nothing, it only keeps the first call
```
...
df5 <- df %>% group_by(VISIT) %>% mutate(mean_words = mean(tokens_CHI + tokens_MOT))

df5 <- df %>% group_by(SUBJ) %>% mutate(mean_chi_words = mean(tokens_CHI))
