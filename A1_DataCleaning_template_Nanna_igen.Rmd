...
```{r}
pacman::p_load(tidyverse,janitor, lme4)
```
...
utter <- read.csv("LU_train.csv")
word_token <- read.csv("token_train.csv")
```
...
utter$VISIT <- str_extract(utter$VISIT, "\\d")

word_token$VISIT <- str_extract(word_token$VISIT, "\\d")
```
...
utter$SUBJ<-str_replace_all(utter$SUBJ,"[.]","")
word_token$SUBJ<-str_replace_all(word_token$SUBJ,"[.]","")
```
...
demo_clean <- subset(demo, select = c("SUBJ", "VISIT", "Diagnosis" , "Ethnicity" , "Gender", "Age", "ADOS", "MullenRaw", "ExpressiveLangRaw", "Socialization" ))
utter_clean <- subset(utter, select = c("SUBJ", "VISIT", "MOT_MLU", "CHI_MLU" ))
token_clean <- subset(word_token, select = c("SUBJ", "VISIT", "types_MOT","types_CHI", "tokens_MOT", "tokens_CHI"))
...
df_all<-merge(demo_clean, utter_clean)
df_all<-merge(df_all, token_clean)
...
```{r}
#Selecting variables
visit1 <- subset(df_all, select=c("SUBJ", "VISIT","ADOS", "MullenRaw","ExpressiveLangRaw", "Socialization"))
#Filtering for visit 1
visit1 <- dplyr::filter(visit1, visit1$VISIT == "1")

#Renaming clinical variables

colnames(visit1)[colnames(visit1)=="ADOS"] <- "ADOS1"
colnames(visit1)[colnames(visit1)=="MullenRaw"] <- "MullenRaw1"
colnames(visit1)[colnames(visit1)=="ExpressiveLangRaw"] <- "ExpressiveLangRaw1"
colnames(visit1)[colnames(visit1)=="Socialization"] <- "Socialization1"

visit1 <- subset(visit1, select=c("SUBJ","ADOS1", "MullenRaw1","ExpressiveLangRaw1", "Socialization1"))

#Merging visit 1 with the clean data

df_all1 <- merge(df_all, visit1,  by = "SUBJ")
...
```{r}
#Gender
df_all1$Gender <- str_replace(df_all1$Gender, "1", "F")
df_all1$Gender <- str_replace(df_all1$Gender, "2", "M")

#SUBJ
df_all1$SUBJ <- as.factor(df_all1$SUBJ)
df_all1$SUBJ <- as.numeric(df_all1$SUBJ)

#Diagnosis
df_all1$Diagnosis <- as.factor(df_all1$Diagnosis)

library(plyr)
df_all1$Diagnosis <- plyr::revalue(df_all1$Diagnosis, c("A" = "ADS", "B" = "TD"))


#Saving
write.csv(df_all1, file = "Autism_clean.csv")
