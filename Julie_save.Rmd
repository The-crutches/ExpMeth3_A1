...
```{r}
lu_train <- read.csv("~/Uni stuff/Expmeth3/ExpMeth3_A1/LU_train.csv")
token_train <- read.csv("~/Uni stuff/Expmeth3/ExpMeth3_A1/token_train.csv")
demo_train <- read.csv("~/Uni stuff/Expmeth3/ExpMeth3_A1/demo_train.csv")
...
finale_frame_demo <- subset(demo_train, select = c("SUBJ", "VISIT", "Ethnicity", "Diagnosis", "Gender", "Age", "ADOS", "MullenRaw", "ExpressiveLangRaw", "Socialization")) 
finale_frame_lu <- subset(lu_train, select = c("SUBJ", "VISIT", "MOT_MLU", "CHI_MLU"))
finale_frame_token <- subset(token_train, select = c("SUBJ", "VISIT", "types_MOT", "types_CHI", "tokens_MOT", "tokens_CHI"))
...
```{r}
first_merged_df <- merge(data.frame(finale_frame_demo, row.names=NULL), data.frame(finale_frame_lu, row.names=NULL), 
  by = 0, all = TRUE)[-1]

finale_merged_df <- merge(data.frame(first_merged_df, row.names=NULL), data.frame(finale_frame_token, row.names=NULL), 
  by = 0, all = TRUE)[-1]
...
```{r}
visit_1 <- subset(finale_merged_df, select = )
