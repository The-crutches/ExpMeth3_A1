...
author: "Studygroup 4, Kristine, Julie, Nanna, Sarah, Martine"
date: "04-09-2020"
...
```{r}
#Renaming variables child. ID and Visit 

demo_train_r <- rename(demo_train, c("SUBJ"="Child.ID", "VISIT"="Visit"))
```
...
demo_train_r$SUBJ <- str_replace_all(demo_train_r$SUBJ, "[.]","")
...
```{r}
#Making seperate dataframes with the right variables
f_demo_train <- subset(demo_train_r, select = c("SUBJ", "VISIT", "Ethnicity", "Diagnosis", "Gender", "Age", "ADOS", "MullenRaw", "ExpressiveLangRaw", "Socialization"))

f_lu <- subset(lu, select = c("SUBJ", "VISIT", "MOT_MLU", "CHI_MLU"))

f_token <- subset(token_train, select = c("SUBJ", "VISIT", "types_MOT", "types_CHI", "tokens_MOT", "tokens_CHI"))
...
f_df_try <- merge(f_demo_train, f_lu)
f_df <- merge(f_df_try, f_token)
...
```{r}
#Making a seperate dataframe for clinical variables
visit1 <- subset(f_df, select=c("SUBJ", "VISIT","ADOS", "MullenRaw","ExpressiveLangRaw", "Socialization"))

#Removing everything else than visit 1
visit1 <- filter(visit1, visit1$VISIT == "1")

#Renaming variable so i can merge the visit1 with f_df without messing up the data  
visit1_r<- rename(visit1, c("ADOS1"="ADOS", "MullenRaw1"="MullenRaw", "ExpressiveLangRaw1"="ExpressiveLangRaw", "Socialization1"="Socialization"))

#Removing visit column 
visit1_r <- subset(visit1_r, select = c("SUBJ","ADOS1", "MullenRaw1","ExpressiveLangRaw1", "Socialization1"))

#Mergin visit1 dataframe with the other dataframe 
final_df <- merge(visit1_r, f_df)
```
...
final_df$SUBJ <- as.factor(final_df$SUBJ)
final_df$SUBJ <- as.numeric(final_df$SUBJ)

#Gender to f - m 
final_df$Gender <- str_replace_all(final_df$Gender, c("1"="F","2"="M"))

#Changing diagnosis labels from A to ASD and B to TD. The dollar in the A is because then you cant do dumb stuff  
final_df$Diagnosis <- str_replace_all(final_df$Diagnosis, c("A$"="ASD","B"="TD"))

#Saving the datafram as autism_df
write.csv(final_df, file = "autism_df")
```
...
#Kids dataframe with above 2.7 morphemes 
final_df$CHI_MLU <- as.numeric(final_df$CHI_MLU)
final_df$SUBJ <- as.numeric(final_df$SUBJ)
mlu2_7 <- final_df %>% group_by(SUBJ) %>% filter(mean(CHI_MLU)>2.7)
#This counts how many children had a mean above 2.7 
cmlu2_7 <- mlu2_7 %>% count(SUBJ)

#Kids under 1.5 mlu in first visit 
mlu1_5 <- final_df %>% filter(VISIT == 1, CHI_MLU<1.5)

#Kids that have not completed all trials. I use the mean function to indicate that it is only those with a mean visit of 3.5 that have completed all trials, because 1+2+3+4+5+6=21, 21/6=3.5 
all_trials <- final_df %>% group_by(SUBJ) %>% filter(mean(VISIT)==3.5)
...
```{r}
#1 produced most words on the 6th visit 
visit6_token <- final_df %>% filter(VISIT==6) %>% arrange(desc(tokens_CHI))

#Kids least words on 1st visit 
visit1_token <- final_df %>% filter(VISIT == 1) %>% arrange(tokens_CHI)
...
```{r}
#Making new dataframe with subj, visit, ASD, mlu, word tokens

asd_df <- final_df %>% filter(Diagnosis=="ASD") %>% subset(select = c("SUBJ", "VISIT", "CHI_MLU","tokens_CHI", "Diagnosis"))

#Question 2: it just adds another column and adds 1 to the varible name 
...
```{r}
#Adding column with mean number of words 
asd_df <- asd_df %>% group_by(SUBJ) %>% mutate(mean_words = mean(tokens_CHI))
